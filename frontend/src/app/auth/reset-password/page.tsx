...
function ResetPasswordForm() {
  ...
  const canSubmit = !!(code && password.length >= 8 && confirm && match && !loading && isLoaded);
    ...
    <div className="glass-panel rounded-2xl p-8 border-[#1B3A5C]/30">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          <span className="text-blue-300">{email || "your email"}</span> and choose a new password
        </p>
      </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full bg-white/5 border rounded-xl pl-10 pr-4 py-3 text-sm text-secondary placeholder:text-muted-foreground focus:outline-none transition-all ${
                confirm && !match ? "border-red-500/50" : "border-white/10 focus:border-[#1B3A5C]/60"
              }`}
            />
          </div>
          {confirm && !match && (
      ...
      <p className="text-center text-xs text-muted-foreground mt-6">
        <Link href="/auth/forgot-password" className="text-blue-300 hover:text-blue-200 font-semibold transition-colors">
          ← Resend reset code
        </Link>
      </p>
    </div>
