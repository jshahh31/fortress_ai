    ...
    <div className="glass-panel rounded-2xl p-8 border-[#1B3A5C]/30">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input type={showPass ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repeat password" required
              className={`w-full bg-white/5 border rounded-xl pl-10 pr-4 py-3 text-sm text-secondary placeholder:text-muted-foreground focus:outline-none transition-all ${
                confirmPassword && !passwordsMatch ? "border-danger/50 focus:border-danger/60" : "border-white/10 focus:border-[#1B3A5C]/60"
              }`} />
          </div>
          {confirmPassword && !passwordsMatch && (
            <p className="text-[10px] text-danger mt-1 font-medium">Passwords do not match</p>
          )}
        </div>
