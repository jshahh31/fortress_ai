...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err?.errors?.[0]?.message || "Something went wrong during Google Sign In.");
    }
  };
    ...
    <div className="glass-panel rounded-2xl p-8 border-[#1B3A5C]/30">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/auth/forgot-password" className="text-xs text-blue-300 hover:text-blue-200 transition-colors font-medium">
            Forgot Password?
          </Link>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#1B3A5C] hover:bg-[#234a72] text-white font-bold text-sm py-3 rounded-xl border border-[#1B3A5C]/60 shadow-lg shadow-[#1B3A5C]/20 transition-all active:scale-[0.98] disabled:opacity-60 flex items-center justify-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 gap-3">
        <button onClick={handleGoogleSignIn} className="flex items-center justify-center gap-2 bg-white/5 border border-white/10 rounded-xl py-2.5 text-sm font-medium text-muted-foreground hover:text-secondary hover:bg-white/10 transition-all">
